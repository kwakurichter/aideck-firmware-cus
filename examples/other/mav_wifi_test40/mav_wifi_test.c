    ...
    while (1) {
        switch (g_mission_state) {
            ...
            case MISSION_WAIT_FOR_STREAMS:
                if (g_range_stream_active) {
                    cpxPrintToConsole(LOG_TO_WIFI, "Data streams active. Waiting for EKF to settle...\n");
                    vTaskDelay(3000 / portTICK_PERIOD_MS);                   
                    cpxPrintToConsole(LOG_TO_WIFI, "Ready for mission command.\n");
            ...
            case MISSION_WAIT_FOR_COMMAND:
                if (is_mission_start) {
                    cpxPrintToConsole(LOG_TO_WIFI, "Mission: Start command received! Setting EKF Origin...\n");
