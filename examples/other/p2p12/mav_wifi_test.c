    ...
    while (1) {
        ...
        switch (g_mission_state) {   
            ...
            case MISSION_TAKEOFF: {
                // 1) Timeout
                if ((g_vehicle_state.altitude_m > 0.95f * tkoff_alt) || (takeoff_ticks++ > TAKEOFF_TICKS)) {
            ...
            case VOODOO_CONTROL: {
                ...
                uint16_t rc_roll = rc_from_norm(roll_target_norm, RC_TRIM, RC_MAX_DELTA);
                uint16_t rc_pitch = rc_from_norm(pitch_target_norm, RC_TRIM, RC_MAX_DELTA);
