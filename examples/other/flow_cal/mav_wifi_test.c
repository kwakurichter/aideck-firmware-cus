        ...
        if (mavlink_parse_char(MAVLINK_COMM_0, rx_byte, &received_msg, &status)) {
            ...
            case MAVLINK_MSG_ID_STATUSTEXT: {
                ...
                case MAVLINK_MSG_ID_OPTICAL_FLOW_RAD: {
                    ...
                    // Forward the data over WiFi in a CSV format for easy plotting.
                    // Format: "OF,time_us,flowX,flowY,gyroX,gyroY,quality,temp,dist_m"
                    cpxPrintToConsole(LOG_TO_WIFI, "OF,%llu,%.4f,%.4f,%.4f,%.4f,%d,%d,%.4f\n",
                                      flow.time_usec,
                                      flow.integrated_x,     // Corresponds to OF.flowX
                                      flow.integrated_y,     // Corresponds to OF.flowY
                                      flow.integrated_xgyro, // Corresponds to OF.bodyX
                                      flow.integrated_ygyro, // Corresponds to OF.bodyY
                                      flow.quality,
                                      flow.temperature,
                                      flow.distance);
                    break;
                ...
                case MAVLINK_MSG_ID_HIGHRES_IMU: {
                    ...
                    // Forward the data over WiFi in a CSV format for easy plotting.
                    // Format: "OF,time_us,flowX,flowY,gyroX,gyroY,quality,temp,dist_m"
                    cpxPrintToConsole(LOG_TO_WIFI, "IMU,%.4f,%.4f,%.4f,%.4f\n",
                                      imu.xgyro,
                                      imu.ygyro,
                                      imu.xacc,
                                      imu.yacc);
