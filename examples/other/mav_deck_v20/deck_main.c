...
void heartbeat_task(void *parameters) {
    while (1) {
        send_heartbeat();
        cpxPrintToConsole(LOG_TO_WIFI, "Heartbeat sent.\n");
...
void start_main_application(void *parameters) {
    ...
    // MAVLink Tasks
    xTask = xTaskCreate(uart_receive_task, "mav_rx_task", configMINIMAL_STACK_SIZE * 8, NULL, tskIDLE_PRIORITY + 2, NULL);
    if (xTask != pdPASS) {
        ...
        if (mavlink_parse_char(MAVLINK_COMM_0, rx_byte, &received_msg, &status)) {
            ...
            switch (received_msg.msgid) {
                case MAVLINK_MSG_ID_HEARTBEAT: {
                    ...
                    cpxPrintToConsole(LOG_TO_WIFI, "<- HEARTBEAT received.\n");
                    break;
                }
                case MAVLINK_MSG_ID_LOCAL_POSITION_NED: {
                    ...
                    cpxPrintToConsole(LOG_TO_WIFI, "<- POSITION received.\n");
                    break;
                }
                case MAVLINK_MSG_ID_PARAM_VALUE: {
                    ...
                    cpxPrintToConsole(LOG_TO_WIFI, "<- PAPRAM received.\n");
                    break;
                }
                default:
