...
typedef enum {
    ...
    MISSION_SET_MODE_ALTHOLD,     
    MISSION_ARM_VEHICLE,
    MISSION_WAIT_FOR_ARM,
    MISSION_PERFORM_TAKEOFF_CLIMB,
    MISSION_HOVER,
    VOODOO_CONTROL,
...
const char* mission_state_to_string(MissionState_t state) {
    switch (state) {
        ...
        case MISSION_SET_MODE_ALTHOLD:      return "SET_MODE_ALTHOLD";        
        case MISSION_ARM_VEHICLE:           return "ARM_VEHICLE";
        case MISSION_WAIT_FOR_ARM:          return "WAIT_FOR_ARM";
        case MISSION_PERFORM_TAKEOFF_CLIMB: return "PERFORM_TAKEOFF_CLIMB";
        ...
        switch (g_mission_state) {
            ...
            case MISSION_WAIT_FOR_COMMAND:
                if (is_mission_start) {
                    ...
                    g_mission_state = MISSION_SET_MODE_ALTHOLD;
                }
                break;
            case MISSION_SET_MODE_ALTHOLD: {
                    mavlink_set_mode(COPTER_MODE_ALT_HOLD);
                    int result = mavlink_wait_for_ack(MAV_CMD_DO_SET_MODE, 2000);
                    if (result == MAV_RESULT_ACCEPTED) {
                        cpxPrintToConsole(LOG_TO_WIFI, "Mission: AltHold mode set. Arming vehicle...\n");
                        g_mission_state = MISSION_ARM_VEHICLE;
                    } else {
                        cpxPrintToConsole(LOG_TO_WIFI, "Mission: FAILED to set AltHold (result: %d).\n", result);
                        g_mission_state = MISSION_DONE;
                    }
                    break;
                }                                
            ...
            case MISSION_WAIT_FOR_ARM:
                ...
                // Reset our throttle variable to hover-level before we start
                s_takeoff_throttle = 500;                

                vTaskDelay(500 / portTICK_PERIOD_MS);
                g_mission_state = MISSION_PERFORM_TAKEOFF_CLIMB;
                break;            
            case MISSION_PERFORM_TAKEOFF_CLIMB: {
                // Check if we have reached the target altitude yet
                if (state->altitude_m >= TAKEOFF_ALT) {
                    cpxPrintToConsole(LOG_TO_WIFI, "Mission: Climb complete. Hovering...\n");
                    hover_counter = 0;
                    g_mission_state = MISSION_HOVER;
                    break;
                } 
                // If we are still on the ground, slowly increase the throttle
                // The loop runs every 500ms, so this adds ~5% throttle per second
                if (s_takeoff_throttle < 700) { // Cap the throttle at 70% to prevent excessive speed
                    s_takeoff_throttle += 25;
                }

                cpxPrintToConsole(LOG_TO_WIFI, "Ramping climb... (Alt: %.2f m, Thr: %d)\n", state->altitude_m, s_takeoff_throttle);
                
                // Send the manual control command with the CURRENT throttle value
                mavlink_send_manual_control(0, 0, s_takeoff_throttle, 0);                
                break;
            }
            case MISSION_HOVER:
                // Command a hover using MANUAL_CONTROL. 500 is mid-throttle.
            ...
            case MISSION_COMMAND_LAND: {
                ...
                int result = mavlink_wait_for_ack(MAV_CMD_NAV_LAND, 2000);
