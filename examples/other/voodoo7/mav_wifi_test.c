...
const char* mission_state_to_string(MissionState_t state) {
    switch (state) {
        ...
        case MISSION_SET_MODE_LOITER:       return "SET_MODE_LOITER";        
...
void uart_receive_task(void *parameters) {
    ...
    while(1) {
        ...
        cpxSendRawData(CPX_T_WIFI_HOST, CPX_F_MAVLINK_DOWNLINK, mavlink_tx_buffer, len);           

        if (mavlink_parse_char(MAVLINK_COMM_0, rx_byte, &received_msg, &status)) {
            // --- Validation Passed: A complete message was received ---         
    ...
    while(1) {
        ...
        if (rxp.route.source == CPX_T_WIFI_HOST && rxp.dataLength > 0) {
            ...
            if (wifiCtrl->cmd == WIFI_CTRL_USER_COMMAND) {
                ...
                const char* start_cmd = "sm";
                const char* reset_cmd = "rm";
                const char* term_cmd = "tm";
                // int received_len = rxp_c.dataLength - sizeof(wifiCtrl->cmd);
                int received_len = rxp.dataLength - 1; // cmd is 1 byte in practice

                // actual string length up to received_len (handles \0 padding safely)
                int cmd_len = my_strnlen((const char*)wifiCtrl->data, received_len);

                // trim common trailing whitespace (handles \r\n etc)
                while (cmd_len > 0) {
                char c = ((char*)wifiCtrl->data)[cmd_len - 1];
                if (c == '\n' || c == '\r' || c == ' ' || c == '\t') cmd_len--;
                else break;
                }       
                ...
                // for (int i = 0; i < rxp_c.dataLength - 1; i++) {
                // printf("%02X ", wifiCtrl->data[i]);
                // }
                // printf("\n");
                // --- DEBUG --- //

                if (memcmp(wifiCtrl->data, "sm", 2) == 0) {
                    printf("<- Received START MISSION command!\n");
                    cpxPrintToConsole(LOG_TO_WIFI, "<- Received START MISSION command!\n");
                    is_mission_start = true;                
                } else if (memcmp(wifiCtrl->data, "rm", 2) == 0) {
                    printf("<- Received RESET MISSION command!\n");
                    cpxPrintToConsole(LOG_TO_WIFI, "<- Received RESET MISSION command!\n");
                    is_mission_reset = true;
                } else if (memcmp(wifiCtrl->data, "tm", 2) == 0) {
                    printf("<- Received TERMINATE MISSION command!\n");
                    cpxPrintToConsole(LOG_TO_WIFI, "<- Received TERMINATE MISSION command!\n");
                    is_mission_terminate = true;
                } else {
                    printf("<- Received command: %.*s (cmd_len=%d, received_len=%d)\n", cmd_len, wifiCtrl->data, cmd_len, received_len);
                }                
            }
        }
    }
    ...
    while (1) {
        ...
        switch (g_mission_state) {
            case MISSION_WAIT_FOR_FC_CONNECT:
                if (g_fc_connected) {
                    printf("FC connected. Requesting data streams...\n");
                    ...
                    vTaskDelay(500 / portTICK_PERIOD_MS);
                    printf("Waiting for FC connection...\n");
                    cpxPrintToConsole(LOG_TO_WIFI, "Waiting for FC connection...\n");
                }
                break;
            case MISSION_WAIT_FOR_STREAMS:
                if (g_pos_stream_active && g_range_stream_active) {
                    printf("Data streams active.\n");
            ...
            case MISSION_WAIT_FOR_COMMAND:
                if (is_mission_start) {
                    printf("Mission: Start command received! Waiting for EKF to settle...\n");
            ...
            case MISSION_SET_EKF_ORIGIN: {
                    mavlink_set_origin(-353632640, 1491652352, 584090);

                    mavlink_send_command_long(MAV_CMD_REQUEST_MESSAGE, MAVLINK_MSG_ID_GPS_GLOBAL_ORIGIN, 0, 0, 0, 0, 0, 0);

                    vTaskDelay(10000 / portTICK_PERIOD_MS);
    
                    //if (is_origin_set) {
                    printf("Mission: Origin set.\n");
                    cpxPrintToConsole(LOG_TO_WIFI, "Mission: Origin set.\n");
                    g_mission_state = MISSION_SET_MODE_LOITER;
                    //} else {
                    //   printf("Mission: Origin set FAILED. Aborting.\n");
                    //    cpxPrintToConsole(LOG_TO_WIFI, "Mission: Origin set FAILED. Aborting.\n");
                    //   g_mission_state = MISSION_DONE;
                    //}                
                    break;
                }                        
            case MISSION_SET_MODE_LOITER: {
                    mavlink_set_mode(COPTER_MODE_LOITER);
                    
                    //int result = mavlink_wait_for_ack(MAV_CMD_DO_SET_MODE, 5000);

                    vTaskDelay(5000 / portTICK_PERIOD_MS);
    
                    //if (result == MAV_RESULT_ACCEPTED) {
                    printf("Mission: LOITER mode set successfully.\n");
                    cpxPrintToConsole(LOG_TO_WIFI, "Mission: LOITER mode set successfully.\n");
                    g_mission_state = MISSION_ARM_VEHICLE;
                    //} else {
                    //    printf("Mission: FAILED to set LOITER mode (result: %d). Aborting.\n", result);
                    //    cpxPrintToConsole(LOG_TO_WIFI, "Mission: FAILED to set LOITER mode (result: %d). Aborting.\n", result);
                    //   g_mission_state = MISSION_SET_MODE_LOITER;
                    //}                
                    break;
                }                         
            case MISSION_ARM_VEHICLE: {
                    ...
                    //if (result == MAV_RESULT_ACCEPTED) {
                    printf("Mission: Vehicle armed successfully.\n");
            ...
            case MISSION_WAIT_FOR_CLIMB:
                if (state->altitude_m >= TAKEOFF_ALT * 0.95) {
                    printf("Mission: Climb complete. Hovering...\n");
            ...
            case MISSION_COMMAND_LAND: {
                ...
                //if (result == MAV_RESULT_ACCEPTED) {
                printf("Mission: Switching to RTL. Mission complete.\n");
                cpxPrintToConsole(LOG_TO_WIFI, "Mission: Switching to RTL. Mission complete.\n");
                g_mission_state = MISSION_DONE;
                //} else {
                //    printf("Mission: FAILED to set RTL (result: %d). Aborting.\n", result);
...
void start_main_application(void *parameters) {
    ...
    if (xTask != pdPASS) {
        printf("FATAL: WiFi debug task creation failed!\n");
