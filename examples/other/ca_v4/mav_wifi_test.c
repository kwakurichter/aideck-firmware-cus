...
// EKF variance thresholds for pre-takeoff health check.
#define EKF_HORIZ_VAR_MAX 0.8f
#define EKF_VERT_VAR_MAX  0.8f
...
#define RSSI_AVOID_THRESHOLD    47.5 // rssi < this → imminent collision risk
#define RSSI_CAUTION_THRESHOLD  50   // rssi < this → peer is nearby, use caution
#define RSSI_CLEAR_THRESHOLD  55     // rssi > this → peer is clear
#define RSSI_STALE_MS          1000  // ignore peers with no P2P data in this window
#define PROX_MONITOR_HZ        50    // proximity check rate
#define PROX_DWELL_MS          500  // hold elevated state this long after the last elevated reading
#define PROX_RECOVER_MS        1000  // time spent in RECOVER before returning to NORMAL after an AVOID
// RSSI filtering
#define RSSI_MED_SAMPLES       5     // median pre-filter window (~0.2 s at 25 Hz)
#define RSSI_LPF_TAU_ATTACK    0.4f  // s — fast, signal magnitude dropping (peer closing)
#define RSSI_LPF_TAU_RELEASE   2.0f  // s — slow, signal magnitude rising  (peer departing)
...
typedef struct {
    ...
    uint8_t rssi;                        // latest raw RSSI from outer frame
    uint8_t  rssi_buf[RSSI_MED_SAMPLES]; // circular buffer for median pre-filter
    uint8_t  rssi_buf_head;              // next write index
    bool     rssi_buf_full;              // true once all slots populated
    float    rssi_filt;                  // asymmetric LPF output (0.0f = uninitialized)
...
_Static_assert(sizeof(p2p_rssi_v1_t) == P2P_RSSI_V1_LEN, "p2p_rssi_v1_t must be 10 bytes");
...
static p2p_parser_state_t g_p2p_rssi_state[P2P_MAX_CHAN];
...
/**
 * Parse one byte of the P2P rssi stream.
 *
 * Returns 1 if a full valid packet was decoded into *out.
 * Returns 0 otherwise.
 *
 ...
 * - Copies the packet into *out only on success.
 */
uint8_t p2p_rssi_parse_char(uint8_t chan, uint8_t c, p2p_rssi_v1_t *out)
{
    if (chan >= P2P_MAX_CHAN || out == NULL) {
        return 0;
    }

    p2p_parser_state_t *st = &g_p2p_rssi_state[chan];

    // If we're idle, wait for STX
    if (st->idx == 0) {
        if (c != P2P_STX_RSSI_V1) {
            return 0;
        }
        st->buf[st->idx++] = c;
        return 0;
    ...
    if (st->idx < P2P_RSSI_V1_LEN) {
        return 0; // still accumulating
    }

    // We have 10 bytes: validate checksum
    bool ok = p2p_fletcher8_ok(st->buf, P2P_RSSI_V1_LEN);
    ...
    // Success
    memcpy(out, st->buf, P2P_RSSI_V1_LEN);
    st->packets_ok++;
 ...
 */
 void mavlink_init_data_streams(void) {
    ...
    // Request EKF_STATUS_REPORT at 2 Hz for pre-takeoff health check
    mavlink_request_data_stream(MAVLINK_MSG_ID_EKF_STATUS_REPORT, 2.0f);
}
...
static inline HealthStatus_t compute_health_status(void)
{
    TickType_t now = xTaskGetTickCount();
    HealthStatus_t h;
    h.range   = g_range_stream_active && ((now - g_range_last_tick)   < pdMS_TO_TICKS(RANGE_FRESH_MS));
    h.flow    = g_flow_healthy        && ((now - g_flow_last_tick)    < pdMS_TO_TICKS(FLOW_FRESH_MS));
    h.battery = g_battery_healthy     && ((now - g_battery_last_tick) < pdMS_TO_TICKS(BAT_FRESH_MS));
    h.ekf     = g_ekf_healthy         && ((now - g_ekf_last_tick)     < pdMS_TO_TICKS(EKF_FRESH_MS));
    return h;
}
...
void request_task(void *parameters) {
    ...
    static bool is_avoid = false;
    static bool is_caution = false;
    ...
    while(1) {
        ...
        ProximityState_t prox = g_proximity_state;
        if (prox == PROX_AVOID)   { is_avoid = true;   is_caution = false; is_clear = false; is_recover = false; }
        if (prox == PROX_CAUTION) { is_caution = true;  is_avoid = false;  is_clear = false; is_recover = false; }
        if (prox == PROX_RECOVER) { is_recover = true;  is_avoid = false;  is_clear = false; is_caution = false; }
        if (prox == PROX_NORMAL)  { is_clear = true;    is_avoid = false;  is_caution = false; is_recover = false; }        

        // Pack fresh health flags into res0: bit0=rng, bit1=flw, bit2=bat, bit3=ekf
        HealthStatus_t health = compute_health_status();
        uint16_t res0 = ((uint16_t)health.range  ) |
                        ((uint16_t)health.flow    << 1) |
                        ((uint16_t)health.battery << 2) |
                        ((uint16_t)health.ekf     << 3) |
                        ((uint16_t)is_clear       << 4) |
                        ((uint16_t)is_recover     << 5) |
                        ((uint16_t)is_caution     << 6) |
                        ((uint16_t)is_avoid       << 7);                        
        ...
        if (mavlink_parse_char(MAVLINK_COMM_0, rx_byte, &received_msg, &status)) {
                ...
                switch (received_msg.msgid) {
                    ...
                    case MAVLINK_MSG_ID_RANGEFINDER: {
                        ...
                        if (range.distance >= 0.001f) {
                            g_range_stream_active = true;
                            g_range_last_tick = xTaskGetTickCount();
                        }
                    ...
                    case MAVLINK_MSG_ID_SYS_STATUS: {
                        ...
                        mavlink_msg_sys_status_decode(&received_msg, &sys);
                        if (sys.voltage_battery > 0) {
                            g_battery_voltage_v = sys.voltage_battery / 1000.0f;
                            g_battery_healthy = (sys.voltage_battery > 4000);
                            g_battery_last_tick = xTaskGetTickCount();
                        }
                        break;
                    }
                    case MAVLINK_MSG_ID_EKF_STATUS_REPORT: {
                        mavlink_ekf_status_report_t ekf;
                        mavlink_msg_ekf_status_report_decode(&received_msg, &ekf);
                        g_ekf_healthy = (ekf.pos_horiz_variance < EKF_HORIZ_VAR_MAX) &&
                                        (ekf.pos_vert_variance  < EKF_VERT_VAR_MAX) &&
            ...
            if (p2p_frame_st == WAIT_MAGIC) {
                ...
                if (rx_byte == P2P_MAGIC) {
                    ...
                    if (p2p_pos_parse_char(0, rx_byte, &pkt3)) {
                        ...
                        if (idx >= 0) {
                            if (pkt3.peer_id == LEADER_SYSTEM_ID && !g_p2p_data_received_once) {
                                g_p2p_data_received_once = true;
                                printf("Peer stream active [%d]", pkt3.peer_id);
                                send_statustextf(MAV_SEVERITY_ALERT, "Peer Stream Active.");
                            }
                    ...
                    if (p2p_rssi_parse_char(0, rx_byte, &pkt4)) {
                        int idx = peer_idx(pkt4.peer_id);
                        if (idx >= 0) {
                            if (pkt4.peer_id == LEADER_SYSTEM_ID && !g_p2p_data_received_once) {
                                g_p2p_data_received_once = true;
                                printf("Peer stream active [%d]", pkt4.peer_id);
                                send_statustextf(MAV_SEVERITY_ALERT, "Peer Stream Active.");
                            }
...
static uint8_t rssi_median(const uint8_t *buf) {
    uint8_t tmp[RSSI_MED_SAMPLES];
    memcpy(tmp, buf, RSSI_MED_SAMPLES);
    for (uint8_t i = 1; i < RSSI_MED_SAMPLES; i++) {
        uint8_t key = tmp[i];
        int8_t  j   = (int8_t)i - 1;
        while (j >= 0 && tmp[j] > key) { tmp[j + 1] = tmp[j]; j--; }
        tmp[j + 1] = key;
    }
    return tmp[RSSI_MED_SAMPLES / 2];
}
...
void proximity_monitor_task(void *parameters) {
    ...
    const float dt            = 1.0f / (float)PROX_MONITOR_HZ;
    const float alpha_attack  = dt / (RSSI_LPF_TAU_ATTACK  + dt);
    const float alpha_release = dt / (RSSI_LPF_TAU_RELEASE + dt);
    ...
    while (1) {
        ...
        for (int i = 0; i < NUM_PEERS; i++) {
            ...
            // Stage 1: push into median circular buffer (only when fresh data is arriving)
            if (!stale) {
                uint8_t h = g_peer_states[i].rssi_buf_head;
                g_peer_states[i].rssi_buf[h] = g_peer_states[i].rssi;
                if (++h >= RSSI_MED_SAMPLES) { h = 0; g_peer_states[i].rssi_buf_full = true; }
                g_peer_states[i].rssi_buf_head = h;
            }

            // Stage 2: asymmetric LPF on median output
            if (!stale && g_peer_states[i].rssi_buf_full) {
                float rssi_med = (float)rssi_median(g_peer_states[i].rssi_buf);
                if (g_peer_states[i].rssi_filt == 0.0f) {
                    g_peer_states[i].rssi_filt = rssi_med;  // seed on first fill
                } else {
                    float alpha = (rssi_med < g_peer_states[i].rssi_filt) ? alpha_attack : alpha_release;
                    g_peer_states[i].rssi_filt += alpha * (rssi_med - g_peer_states[i].rssi_filt);
                }
            }

            // Use filtered RSSI once the buffer is primed, raw RSSI before that
            float rssi_for_class = g_peer_states[i].rssi_buf_full
                                   ? g_peer_states[i].rssi_filt
                                   : (float)g_peer_states[i].rssi;

            // Derive proximity state from filtered RSSI (stale peers treated as NORMAL)
            ProximityState_t raw;
            if (stale) {
                raw = PROX_NORMAL;
            } else {
                if (rssi_for_class < RSSI_AVOID_THRESHOLD) {
                    raw = PROX_AVOID;
                } else if (rssi_for_class < RSSI_CAUTION_THRESHOLD) {
                    raw = PROX_CAUTION;
                } else {
                    raw = PROX_NORMAL;
                }
            ...
            if (cur == PROX_RECOVER) {
                ...
                if (raw == PROX_AVOID) {
                    ...
                    g_peer_states[i].prox_last_elevated = now;
                           ...
                           (stale || rssi_for_class > RSSI_CLEAR_THRESHOLD)) {
...
void autonomous_mission_task(void *parameters) {
    ...
    const int DISARM_TIME_MS = 1000;
    const int DISARM_TICKS = (DISARM_TIME_MS / MISSION_DT_MS);
    ...
    const int EXP_TIMEOUT_MS = 8000;        // 3-second timeout 
    ...
    while (1) {
        ...
        switch (g_mission_state) {   
            ...
            case MISSION_TAKEOFF: {
                bool alt_reached = (g_vehicle_state.position_ned[2] < -(0.9f * tkoff_alt));
                ...
                // Ramp throttle 1450 → 1550 over TAKEOFF_RAMP_TICKS
                uint16_t tkoff_thr = (uint16_t)(THR_TKOFF_START + (takeoff_ticks * (THR_TKOFF_END - THR_TKOFF_START) / TAKEOFF_RAMP_TICKS));
                if (tkoff_thr >= 1550) tkoff_thr = 1550;
                mavlink_send_rc_override_4ch(STM32_SYSTEM_ID, STM32_COMPONENT_ID, 1500, 1500, tkoff_thr, 1500);
                takeoff_ticks++;
            ...
            case EXP_CONTROL: {
                ...
                if (g_proximity_state == PROX_AVOID) {
                    // Hold horizontal position. Move up (~25 cm).
                    mavlink_set_local_target(center_n, center_e, (center_d - 0.25f), 0, 0, -0.25f, center_yaw);
                    break;
                }
                ...
                if (g_proximity_state == PROX_RECOVER) {
                    // Move back down
                    float vz = (g_vehicle_state.position_ned[2] < center_d - 0.10f) ? 0.25f : 0.0f;
            ...
            case MISSION_WAIT_FOR_LAND:
                if ((g_vehicle_state.position_ned[2] > -2.0f * LAND_ALT) || (land_ticks >= LAND_TICKS)) {
                    land_ticks = 0;
                    disarm_ticks = 0;
