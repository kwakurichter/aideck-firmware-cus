...
static void test_gap8(void)
{
    ...
    while(1)
    {
        ...
        // Write the entire mavlink_heartbeat array to the UART
        pi_uart_write(&uart_device, mavlink_heartbeat, sizeof(mavlink_heartbeat));

        // Increment the sequence number for the next packet
        mavlink_heartbeat[2]++;

        // NOTE: In a real implementation, you would also recalculate the checksum here.
        // For this test, we are skipping that step.
