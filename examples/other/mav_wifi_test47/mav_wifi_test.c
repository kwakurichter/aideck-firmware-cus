    ...
    while (1) {
        ...
        switch (g_mission_state) {
            ...
            case MISSION_WAIT_FOR_ARM:
                cpxPrintToConsole(LOG_TO_WIFI, "Sending zero thrust to unlock throttle...\n");
                mavlink_set_attitude_target(0.0, 0.0, 0.0, 0.0); // Send zero thrust
            ...
            case MISSION_PERFORM_TAKEOFF_CLIMB: {
                ...
                if (state->altitude_m >= TAKEOFF_ALT) {
                    ...
                    // We are still climbing. Command a gentle ascent.
                    // A thrust of ~0.65 should be enough for a steady climb.
                    // We command neutral roll/pitch to ensure it goes straight up.
                    cpxPrintToConsole(LOG_TO_WIFI, "Climbing... (Alt: %.2f m)\n", state->altitude_m);
                    mavlink_set_attitude_target(0.0, 0.0, 0.0, 0.65f);
                }
                break;
            }
            case MISSION_HOVER:
                // Command a neutral attitude to prevent drift in AltHold
                mavlink_set_attitude_target(0.0, 0.0, 0.0, 0.5); // Roll=0, Pitch=0, YawRate=0, Thrust=50%                
