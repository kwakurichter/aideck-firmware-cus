...
void start_mavlink_system(void) {
    ...
    // The pmsis_kickoff function will start the FreeRTOS scheduler.
    // The code will not proceed beyond this point.
}

int main(void) {
    printf("\n\n\t *** PMSIS FreeRTOS MAVLink Test ***\n\n");

    // --- THE FIX: Add full hardware initialization before starting the system ---
    // This is required for FreeRTOS and peripherals to function correctly.

    // 1. Set the chip frequency to a known value for stable timing.
    pi_freq_set(PI_FREQ_DOMAIN_FC, 250000000);

    // 2. Initialize the Board Support Package (configures pins, etc.)
    pi_bsp_init();
