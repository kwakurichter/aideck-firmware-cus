        ...
        switch (g_mission_state) {   
            ...
            case VOODOO_CONTROL: {
                ...
                if (peer_pitch < -VOODOO_PITCH_THRESHOLD) {
                    ...
                    voodoo_ticks = 0;
                    mavlink_send_rc_override_4ch(STM32_SYSTEM_ID, STM32_COMPONENT_ID, 1500, 1100, 1500, 1500);
                }
                else if (peer_pitch > VOODOO_PITCH_THRESHOLD) {
                    // Move Backward (Pitch stick back is positive X)
                    voodoo_ticks = 0;
                    mavlink_send_rc_override_4ch(STM32_SYSTEM_ID, STM32_COMPONENT_ID, 1500, 1900, 1500, 1500);
                }
                else if (peer_roll > VOODOO_ROLL_THRESHOLD) {
                    // Move Right (Roll stick right is positive Y)
                    voodoo_ticks = 0;
                    mavlink_send_rc_override_4ch(STM32_SYSTEM_ID, STM32_COMPONENT_ID, 1900, 1500, 1500, 1500);
                }
                else if (peer_roll < -VOODOO_ROLL_THRESHOLD) {
                    // Move Left (Roll stick left is negative Y)
                    voodoo_ticks = 0;
                    mavlink_send_rc_override_4ch(STM32_SYSTEM_ID, STM32_COMPONENT_ID, 1100, 1500, 1500, 1500);
                }
                else {
