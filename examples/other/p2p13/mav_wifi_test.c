...
static volatile bool g_param_kp_pos = false;
static volatile bool g_param_kp_vel = false;
static volatile bool g_param_ki_vel = false;
static volatile bool g_param_filt = false;
static volatile bool g_param_hov_time = false;
static volatile bool g_param_tkoff_alt = false;
static volatile bool g_param_i_max = false;
static volatile bool g_param_v_max = false;
...
_Static_assert(sizeof(p2p_mstate_v1_t) == P2P_MS_V1_LEN, "p2p_ms_v1_t must be 13 bytes");
 ...
 */
void setupWiFi(void) {
    static char ssid[] = "AideckDebugAP3";
...
{
    return g_param_kp_pos && g_param_kp_vel && g_param_ki_vel && g_param_filt && g_param_hov_time && g_param_tkoff_alt && g_param_i_max && g_param_v_max;
}
...
void request_task(void *parameters) {
    ...
    static bool n_flow = false, n_kpp = false, n_kpv = false, n_kiv = false, n_filt = false, n_hov = false, n_tkoff = false, n_imax = false, n_vmax = false;
    ...
    while(1) {
        ...
        send_statustextf(MAV_SEVERITY_ALERT, "MS1,st=%u,val=%u,seq=%u", g_vehicle_state.mission_state, value, sequence);
        ...
        if (g_fc_connected) {
            ...
            if (!g_params_received && (param_ticks % 2 == 0)) {
                ...
                if (!g_param_kp_pos) {
                    request_parameter("GUID_KP_POS");
                } else if (!g_param_kp_vel) {
                    request_parameter("GUID_KP_VEL");
                } else if (!g_param_ki_vel) {
                    request_parameter("GUID_KI_VEL");
                } else if (!g_param_filt) {
                    request_parameter("GUID_FILT");
                } else if (!g_param_hov_time) {
                    request_parameter("GUID_HOV_TIME");
                } else if (!g_param_i_max) {
                    request_parameter("GUID_I_MAX");
                } else if (!g_param_v_max) {
                    request_parameter("GUID_V_MAX");
            ...
            if (g_param_kp_pos && !n_kpp)  {
                send_statustextf(MAV_SEVERITY_ALERT, "AI: got KP POS=%.3f", (double)g_controller_state.kp_pos);
                n_kpp  = true;
            }
            if (g_param_kp_vel && !n_kpv)  { 
                send_statustextf(MAV_SEVERITY_ALERT, "AI: got KP VEL=%.3f", (double)g_controller_state.kp_vel);
                n_kpv  = true;
            }
            if (g_param_ki_vel && !n_kiv)  { 
                send_statustextf(MAV_SEVERITY_ALERT, "AI: got KI VEL=%.3f", (double)g_controller_state.ki_vel);
                n_kiv  = true;
            }
            if (g_param_filt && !n_filt) {
                send_statustextf(MAV_SEVERITY_ALERT, "AI: got FILT=%.3f", (double)g_controller_state.filt);
                n_filt = true;
            }  
            ...
            if (g_param_i_max && !n_imax) {
                send_statustextf(MAV_SEVERITY_ALERT, "AI: got I MAX=%.3f", (double)g_controller_state.i_max);
                n_imax = true;
            }  
            if (g_param_v_max && !n_vmax) {
                send_statustextf(MAV_SEVERITY_ALERT, "AI: got V MAX=%.3f", (double)g_controller_state.v_max);
                n_vmax = true;
            }                                                  
        ...
        if (mavlink_parse_char(MAVLINK_COMM_0, rx_byte, &received_msg, &status)) {
                ...
                switch (received_msg.msgid) {
                    ...
                    case MAVLINK_MSG_ID_PARAM_VALUE: {
                        ...
                        if (strcmp(name, "GUID_KP_POS") == 0) {
                            g_controller_state.kp_pos = param.param_value;
                            g_param_kp_pos = true;
                        }
                        else if (strcmp(name, "GUID_KP_VEL") == 0) {
                            g_controller_state.kp_vel = param.param_value;
                            g_param_kp_vel = true;
                        }
                        else if (strcmp(name, "GUID_KI_VEL") == 0) {
                            g_controller_state.ki_vel = param.param_value;
                            g_param_ki_vel = true;
                        }
                        else if (strcmp(name, "GUID_FILT") == 0) {
                            g_controller_state.filt = param.param_value;
                            g_param_filt = true;
                        }
                        else if (strcmp(name, "GUID_HOV_TIME") == 0) {
                        ...
                        else if (strcmp(name, "GUID_I_MAX") == 0) {
                            g_controller_state.i_max = param.param_value;
                            g_param_i_max = true;
                        }
                        else if (strcmp(name, "GUID_V_MAX") == 0) {
                            g_controller_state.v_max = param.param_value;
                            g_param_v_max = true;
                        }                                                                         
                        break;
                    }
    ...
    while (1) {
        ...
        switch (g_mission_state) {   
            ...
            case VOODOO_CONTROL: {
                ...
                // Hysteresis latch
                if (!x_guard_active && fabsf(dn) > BOX_OUTER_M) x_guard_active = true;
                if ( x_guard_active && fabsf(dn) < BOX_INNER_M) x_guard_active = false;

                if (!y_guard_active && fabsf(de) > BOX_OUTER_M) y_guard_active = true;
                if ( y_guard_active && fabsf(de) < BOX_INNER_M) y_guard_active = false;  
                
                // Safety corrections in normalized stick units
                float pitch_guard_norm = 0.0f;
                float roll_guard_norm  = 0.0f;    

                // Sign depends on RC convention
                if (x_guard_active) {
                    pitch_guard_norm = (dn > 0.0f) ? +GUARD_CMD_NORM : -GUARD_CMD_NORM;
                }
                if (y_guard_active) {
                    roll_guard_norm = (de > 0.0f) ? -GUARD_CMD_NORM : +GUARD_CMD_NORM;
                }                  
                // --- SAFETY --- // 
                ...
                // 1) Map peer attitude to normalized command [-1, 1]
                float roll_target_norm = apply_deadband_and_scale(peer_roll, VOODOO_DEADBAND_DEG, VOODOO_IN_MAX_DEG);
                float pitch_target_norm = apply_deadband_and_scale(peer_pitch, VOODOO_DEADBAND_DEG, VOODOO_IN_MAX_DEG);

                // Combine with voodoo command and saturate
                float final_roll_norm  = clampf(roll_target_norm  + roll_guard_norm,  -1.0f, 1.0f);
                float final_pitch_norm = clampf(pitch_target_norm + pitch_guard_norm, -1.0f, 1.0f);                

                // axis inversion (off by default)
                if (INVERT_ROLL_CMD) {
                    roll_target_norm = -roll_target_norm;
                }
                if (INVERT_PITCH_CMD) {
                    pitch_target_norm = -pitch_target_norm;
                }

                // 3) Convert normalized commands to RC override PWM
                uint16_t rc_roll = rc_from_norm(final_roll_norm, RC_TRIM, RC_MAX_DELTA);
                uint16_t rc_pitch = rc_from_norm(final_pitch_norm, RC_TRIM, RC_MAX_DELTA);

                // 4) Reset timeout whenever there is meaningful peer command activity
                if ((fabsf(roll_target_norm) > 0.01f) || (fabsf(pitch_target_norm) > 0.01f)) {
                    voodoo_ticks = 0;
                } else {
                    voodoo_ticks++;
                }

                // 5) Send simultaneous roll + pitch override, keep throttle/yaw centered
                mavlink_send_rc_override_4ch(STM32_SYSTEM_ID, STM32_COMPONENT_ID, rc_roll, rc_pitch, 1500, 1500);
