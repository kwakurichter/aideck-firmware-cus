...
#define WIFI_LOG_BUFFER_SIZE 1000
#define WIFI_LOG_SEND_THRESHOLD 900 
...
static char g_wifi_log_buffer[WIFI_LOG_BUFFER_SIZE];
static volatile int g_wifi_log_buffer_idx = 0;
...
static volatile bool g_of_stream_active = false;
static volatile bool g_imu_stream_active = false;
...
const char* mission_state_to_string(MissionState_t state) {
    switch (state) {
        ...
        case MISSION_DONE:                  return "MISSION_DONE";
 ...
 */
 void mavlink_init_data_streams(void) {
    ...
    // Request OPTICAL_FLOW_RAD at 10 Hz
    mavlink_request_data_stream(MAVLINK_MSG_ID_OPTICAL_FLOW_RAD, 2.0f); 
    
    // Request IMU at 10 Hz
    mavlink_request_data_stream(MAVLINK_MSG_ID_HIGHRES_IMU, 2.0f); 
}
...
// Helper function to send the buffer and reset it
void send_and_reset_log_buffer() {
    if (g_wifi_log_buffer_idx > 0) {
        
        // IMPORTANT: Ensure the buffer is a valid, null-terminated C-string
        // before passing it to a function that expects a string.
        g_wifi_log_buffer[g_wifi_log_buffer_idx] = '\0';

        // Use the existing, thread-safe function to send our entire buffer.
        // The "%s" format specifier tells it to send the contents of our buffer as a string.
        cpxPrintToConsole(LOG_TO_WIFI, "%s", g_wifi_log_buffer);

        // Reset the buffer index for the next batch
        g_wifi_log_buffer_idx = 0;
    }
}
...
void uart_receive_task(void *parameters) {
    ...
    while(1) {
        ...
        if (mavlink_parse_char(MAVLINK_COMM_0, rx_byte, &received_msg, &status)) {
            ...
            case MAVLINK_MSG_ID_STATUSTEXT: {
                ...
                case MAVLINK_MSG_ID_OPTICAL_FLOW_RAD: {
                    if (!g_of_stream_active) {
                        g_of_stream_active = true;
                    }

                    mavlink_optical_flow_rad_t flow;
                    mavlink_msg_optical_flow_rad_decode(&received_msg, &flow);
                
                    // Write the formatted string into our buffer instead of sending it directly
                    int bytes_written = snprintf(&g_wifi_log_buffer[g_wifi_log_buffer_idx],
                        WIFI_LOG_BUFFER_SIZE - g_wifi_log_buffer_idx,
                        "OF,%llu,%.4f,%.4f,%.4f,%.4f,%d,%d,%.4f\n",
                        flow.time_usec, flow.integrated_x, flow.integrated_y,
                        flow.integrated_xgyro, flow.integrated_ygyro,
                        flow.quality, flow.temperature, flow.distance);

                    // If bytes were written successfully, advance our buffer index
                    if (bytes_written > 0) {
                        g_wifi_log_buffer_idx += bytes_written;
                    }                        
                    break;
                }
                case MAVLINK_MSG_ID_HIGHRES_IMU: {
                    if (!g_imu_stream_active) {
                        g_imu_stream_active = true;
                    }

                    mavlink_highres_imu_t imu;
                    mavlink_msg_highres_imu_decode(&received_msg, &imu);
                
                    // Write the IMU data to the buffer as well
                    int bytes_written = snprintf(&g_wifi_log_buffer[g_wifi_log_buffer_idx],
                        WIFI_LOG_BUFFER_SIZE - g_wifi_log_buffer_idx,
                        "IMU,%llu,%.4f,%.4f,%.4f,%.4f\n",
                        imu.time_usec, imu.xgyro, imu.ygyro, imu.xacc, imu.yacc);
        ...
        // After processing a message, check if the buffer is ready to be sent
        if (g_wifi_log_buffer_idx > WIFI_LOG_SEND_THRESHOLD) {
            send_and_reset_log_buffer();
        }
    }
}
...
void wifi_debug_task(void *parameters) {
    while(1) {
        // This task now has a new purpose: flush the log buffer periodically
        send_and_reset_log_buffer(); // This is the helper function from Step 2
        vTaskDelay(2000 / portTICK_PERIOD_MS); // Flush every 2 seconds
    }
}
...
void autonomous_mission_task(void *parameters) {
    ...
    const int MAX_STREAM_RETRIES = 20; // 3-second timeout
    ...
    while (1) {
        switch (g_mission_state) {
            ...
            case MISSION_WAIT_FOR_STREAMS:
                if (g_of_stream_active && g_imu_stream_active) {
...
void start_main_application(void *parameters) {
    ...
    xTask = xTaskCreate(autonomous_mission_task, "mission_task", configMINIMAL_STACK_SIZE * 2, NULL, tskIDLE_PRIORITY + 1, NULL);
    if (xTask != pdPASS) {
