...
void uart_receive_task(void *parameters) {
    ...
    while(1) {
        ...
        if (mavlink_parse_char(MAVLINK_COMM_0, rx_byte, &received_msg, &status)) {
            ...
            if (received_msg.sysid == PEER_SYSTEM_ID) {
                if (received_msg.msgid == MAVLINK_MSG_ID_ATTITUDE) {
                    ...
                    printf("<- P2P MSG with ID: %d\n", received_msg.msgid);
                    cpxPrintToConsole(LOG_TO_WIFI, "<- P2P MSG with ID: %d\n", received_msg.msgid);
                }
                return; // Don't want to send to GCS
            }
        ...
        switch (g_mission_state) {
            ...
            case MISSION_SET_MODE_GUIDED: {
                    ...
                    if (result == MAV_RESULT_ACCEPTED) {
                        printf("Mission: Guided mode set successfully.\n");
                        cpxPrintToConsole(LOG_TO_WIFI, "Mission: Guided mode set successfully.\n");
                        g_mission_state = MISSION_ARM_VEHICLE;
                    } else {
                        printf("Mission: FAILED to set Guided mode (result: %d). Aborting.\n", result);
            ...
            case MISSION_HOVER:
                ...
                if (g_p2p_data_received_once) {
                    printf("Mission: Peer data detected. Entering Voodoo Control.\n");
