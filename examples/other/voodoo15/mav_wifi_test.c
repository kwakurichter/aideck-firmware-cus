...
// Simple P gains
#define KP_POS_TO_RC 180.0f   // RC delta per meter of error (start ~150-250)
...
typedef struct {
    ...
    int   wp_index;          // 0..3
    int   settle_ticks;      // counts time within radius
} PathTracker_t;
...
const char* mission_state_to_string(MissionState_t state) {
    switch (state) {
        ...
        case MISSION_PATH_TRACK:            return "PATH_TRACK";
...
static inline int16_t apply_deadband(int16_t v, int16_t db) {
    if (v > -db && v < db) return 0;
    return v;
}

static void get_square_wp(float origin_n, float origin_e, int wp_idx, float *wp_n, float *wp_e)
{
    // 1x1 square path, starting at origin, then +E, then +N, then -E back, then -N back
    // Only 4 corners (wp_idx 0..3)
    switch (wp_idx) {
    default:
    case 0: *wp_n = origin_n;               *wp_e = origin_e;               break;
    case 1: *wp_n = origin_n;               *wp_e = origin_e + SQUARE_SIZE_M; break;
    case 2: *wp_n = origin_n + SQUARE_SIZE_M; *wp_e = origin_e + SQUARE_SIZE_M; break;
    case 3: *wp_n = origin_n + SQUARE_SIZE_M; *wp_e = origin_e;               break;
    }
}

static bool path_track_step_1m_square(void)
{
    // Current position (N/E) in meters
    float cur_n = g_vehicle_state.position_ned[0];
    float cur_e = g_vehicle_state.position_ned[1];

    // Initialize origin once (when you decide you are stable)
    if (!g_path.origin_set) {
        g_path.origin_n = cur_n;
        g_path.origin_e = cur_e;
        g_path.origin_set = true;
        g_path.wp_index = 1;       // start moving to corner 1 (since corner 0 is current origin)
        g_path.settle_ticks = 0;
    }

    // Current waypoint
    float wp_n, wp_e;
    get_square_wp(g_path.origin_n, g_path.origin_e, g_path.wp_index, &wp_n, &wp_e);

    // Position error
    float err_n = wp_n - cur_n;
    float err_e = wp_e - cur_e;

    // Arrival check
    float dist2 = err_n*err_n + err_e*err_e;
    if (dist2 <= (WP_RADIUS_M * WP_RADIUS_M)) {
        g_path.settle_ticks++;
        // Hover centered when arrived
        mavlink_send_rc_override_4ch(STM32_SYSTEM_ID, STM32_COMPONENT_ID,
                                     RC_CENTER, RC_CENTER, RC_CENTER, RC_CENTER);

        // Stayed inside radius long enough?
        if (g_path.settle_ticks >= SETTLE_TICKS) { // 10Hz => 100ms per tick
            g_path.settle_ticks = 0;
            g_path.wp_index++;
            if (g_path.wp_index > 3) {
                // Square complete
                return true;
            }
        }
    ...
    // Not arrived -> reset settle counter
    g_path.settle_ticks = 0;

    // Convert position error to RC deltas (P controller)
    // Map: East error -> roll, North error -> pitch
    int16_t d_roll  = (int16_t)(KP_POS_TO_RC * err_e);
    int16_t d_pitch = (int16_t)(KP_POS_TO_RC * err_n);

    // Deadband + clamp to +/-200
    d_roll  = apply_deadband(d_roll,  RC_DEADBAND);
    d_pitch = apply_deadband(d_pitch, RC_DEADBAND);

    d_roll  = clamp_i16(d_roll,  -RC_MAX_DELTA, RC_MAX_DELTA);
    d_pitch = clamp_i16(d_pitch, -RC_MAX_DELTA, RC_MAX_DELTA);

    // Compose RC overrides
    uint16_t ch1_roll  = (uint16_t)(RC_CENTER + d_roll);
    uint16_t ch2_pitch = (uint16_t)(RC_CENTER + d_pitch);
...
void autonomous_mission_task(void *parameters) {
    ...
    while (1) {
        ...
        if (flow_lost && /* only during maneuver */ 
            (g_mission_state == MISSION_PATH_TRACK || g_mission_state == MISSION_HOVER)) {
        ...
        switch (g_mission_state) {   
            ...
            case MISSION_HOVER:
                if (hover_ticks >= HOVER_TICKS) {
                    ...
                    g_mission_state = MISSION_PATH_TRACK;
                } else {
                    hover_ticks++;
                }
                break;
            case MISSION_PATH_TRACK: {
                bool done = path_track_step_1m_square();
                if (done) {
                    printf("Path complete -> LAND\n");
                    g_mission_state = MISSION_COMMAND_LAND;
                }
                break;                
            }
