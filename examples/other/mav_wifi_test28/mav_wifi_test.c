...
#define MAX_ARM_RETRIES 10
#define MAX_STREAM_RETRIES 6
    ...
    while (current_state != MISSION_DONE) {
        switch (current_state) {
            ...
            case MISSION_COMMAND_ARM:
                mavlink_arm_vehicle();
                arm_retries = 0; // Reset retry counter
                current_state = MISSION_WAIT_FOR_ARM;
                break;                        
            case MISSION_WAIT_FOR_ARM:
                if (g_vehicle_state.is_armed) {
                    ...
                    if (arm_retries >= MAX_ARM_RETRIES) {
                        cpxPrintToConsole(LOG_TO_WIFI, "Mission: FAILED to arm after %d seconds. Aborting.\n", MAX_ARM_RETRIES / 2);
                        current_state = MISSION_DONE;
                    } else {
                        // If we haven't timed out, send the arm command again and increment counter
                        cpxPrintToConsole(LOG_TO_WIFI, "Mission: Waiting for arm... (attempt %d/%d)\n", arm_retries + 1, MAX_ARM_RETRIES);
                        mavlink_arm_vehicle(); // Re-send the arm command
                        arm_retries++;                        
                    }
                }
                break;
            ...
            case MISSION_GOTO_FORWARD: {
                ...
                int result = mavlink_wait_for_ack(MAVLINK_MSG_ID_SET_POSITION_TARGET_LOCAL_NED, 2000);

                if (result == MAV_RESULT_ACCEPTED) {
                    cpxPrintToConsole(LOG_TO_WIFI, "Mission: Target position accepted.\n");
                    current_state = MISSION_SET_MODE_RTL;
                } else {
                    cpxPrintToConsole(LOG_TO_WIFI, "Mission: FAILED to set target position (result: %d). Aborting.\n", result);
                    current_state = MISSION_DONE;
                }                
                break;
            }    
