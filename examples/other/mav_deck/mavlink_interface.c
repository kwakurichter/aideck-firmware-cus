...
// Defined in mav_wifi_test.c or elsewhere
#define GAP8_SYSTEM_ID 1
#define GAP8_COMPONENT_ID 1

static VehicleState_t g_vehicle_state = {0}; // A single, shared structure to hold the vehicle's state
static struct pi_device uart_device; // UART device handle
static xQueueHandle mavlink_tx_queue; // Queue for outgoing MAVLink messages
static uint8_t send_buffer[MAVLINK_MAX_PACKET_LEN];

static void mavlink_init_data_streams(void);
 ...
 */
static void uart_transmit_task(void *parameters) {
...
/**
 * @brief Initializes serial tasks.
 */
VehicleState_t* mavlink_interface_init(void) {
    ...
    if (mavlink_tx_queue == NULL) {
        printf("FATAL: MAVLink TX queue creation failed!\n");
        pmsis_exit(-1);
    }

    // 3. Create the FreeRTOS tasks
    BaseType_t xTask;
    xTask = xTaskCreate(uart_receive_task, "mav_rx_task", configMINIMAL_STACK_SIZE * 4, NULL, tskIDLE_PRIORITY + 1, NULL);
    if (xTask != pdPASS) {
        printf("FATAL: MAVLink UART receive task creation failed!\n");
    }
    xTask = xTaskCreate(uart_transmit_task, "mav_tx_task", configMINIMAL_STACK_SIZE * 4, NULL, tskIDLE_PRIORITY + 1, NULL);
    if (xTask != pdPASS) {
        printf("FATAL: MAVLink UART transmit task creation failed!\n");
    }        

    // 4. Request the data streams
    mavlink_init_data_streams();

    printf("MAVLink interface initialized.\n");

    // 5. Return a pointer to the shared state structure
