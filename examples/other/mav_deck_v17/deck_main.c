...
SemaphoreHandle_t g_state_mutex; // Handle for the mutex that will protect g_vehicle_state
...
void start_main_application(void *parameters) {
    ...
    if (g_state_mutex == NULL) {
        printf("FATAL: Could not create state mutex!\n");
        pmsis_exit(-1);
    }    
...
void uart_receive_task(void *parameters) {
    ...
    while(1) {
        ...
        if (mavlink_parse_char(MAVLINK_COMM_0, rx_byte, &received_msg, &status)) {
            ...
            // Take the mutex before writing to the shared state
            if (xSemaphoreTake(g_state_mutex, (TickType_t)10) == pdTRUE) {            
