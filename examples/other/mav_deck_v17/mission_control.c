...
void autonomous_mission_task(void *parameters) {
    // This task will need access to the mutex handle
    ...
    while (current_state != MISSION_DONE) {
        switch (current_state) {
            ...
            case MISSION_WAIT_FOR_ARM: {
                bool is_armed = false; // Local variable

                if (xSemaphoreTake(g_state_mutex, (TickType_t)10) == pdTRUE) {
                    is_armed = mavlink_is_armed(); // mavlink_is_armed() reads the global state
            ...
            case MISSION_COMMAND_TAKEOFF:
                ...
                float current_alt = 0.0f; // Local variable
                if (xSemaphoreTake(g_state_mutex, (TickType_t)10) == pdTRUE) {
                    current_alt = state->altitude_m;
                    xSemaphoreGive(g_state_mutex);
                }
