...
static volatile bool g_range_stream_active = false;   // rangefinder has returned a non-zero reading
...
void request_task(void *parameters) {
    ...
    while(1) {
        ...
        // Pack health flags into res0: bit0=rng, bit1=flw, bit2=bat, bit3=ekf
        uint16_t res0 = ((uint16_t)g_range_stream_active      ) |
                        ((uint16_t)g_flow_healthy          << 1) |
                        ((uint16_t)g_battery_healthy       << 2) |
                        ((uint16_t)g_ekf_healthy           << 3);
...
void autonomous_mission_task(void *parameters) {
    ...
    const int TAKEOFF_TIME_MS = 3500;
        ...
        switch (g_mission_state) {   
            case MISSION_WAIT_FOR_COMMAND:
                if (is_mission_start && g_params_received && g_p2p_data_received_once) {
                    if (g_range_stream_active && g_flow_healthy && g_battery_healthy && g_ekf_healthy) {
            ...
            case MISSION_TAKEOFF: {
                bool ramp_done = (takeoff_ticks >= TAKEOFF_RAMP_TICKS);
                bool alt_reached = (g_vehicle_state.altitude_m > 0.9f * tkoff_alt);

                if (ramp_done || alt_reached || (takeoff_ticks > TAKEOFF_TICKS)) {
