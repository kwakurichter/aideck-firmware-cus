    ...
    while(1) {
        ...
        if (rxp.route.source == CPX_T_WIFI_HOST && rxp.dataLength > 0) {
            ...
            if (wifiCtrl->cmd == WIFI_CTRL_USER_COMMAND) {
                // Print the received command back to the WiFi console
                // Note: The data from the python script is not null-terminated automatically
                cpxPrintToConsole(LOG_TO_WIFI, "Received command: %.*s\n",
                                  rxp.dataLength - sizeof(wifiCtrl->cmd), // length of the string
                                  wifiCtrl->data);                       // the string itself
            }
        }
    }
}
