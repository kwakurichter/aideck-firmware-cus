...
const char* mission_state_to_string(MissionState_t state) {
    switch (state) {
        ...
        case MISSION_MOVE_BACK:             return "MOVE_BACK";
        ...
        case MISSION_DISARM:                return "DISARM_VEHICLE";
    ...
    while (1) {
        ...
        switch (g_mission_state) {   
            case MISSION_WAIT_FOR_LOITER:
                ...
                //  if (g_vehicle_state.mode == COPTER_MODE_GUIDED_NOGPS) {
                    printf("Mission: Starting Mission!\n");
                    //cpxPrintToConsole(LOG_TO_WIFI, "Mission: Starting Misiion!\n");
            ...
            case MISSION_HOVER:
                if (hover_ticks >= HOVER_TICKS) {
                    hover_ticks = 0;
                    move_ticks = 0;
                    printf("Mission: Hover complete. Initiating Movement.\n");
                    //cpxPrintToConsole(LOG_TO_WIFI, "Mission: Hover complete. Initiating Movement.\n");
                    g_mission_state = MISSION_MOVE_LEFT;
                } else {
                    //printf("Hovering...\n");
                    //cpxPrintToConsole(LOG_TO_WIFI, "Hovering...\n");
                    hover_ticks++;
                }
                break;
            case MISSION_MOVE_LEFT:
                ...
                // mavlink_set_attitude_target(-0.261799, 0, 0, 0.5);

                if (move_ticks >= MOVE_TICKS) {
                    printf("Mission: Left move complete.\n");
                    move_ticks = 0;
            ...
            case MISSION_BRAKE_1:
                if (brake_ticks >= BRAKE_TICKS) {
                    ...
                    g_mission_state = MISSION_MOVE_BACK;
                } else {
                    //printf("Braking...\n");
            ...
            case MISSION_MOVE_BACK:
                ...
                // mavlink_set_attitude_target(0, -0.261799, 0, 0.5);

                if (move_ticks >= MOVE_TICKS) {
                    printf("Mission: Back move complete.\n");
                    move_ticks = 0;
            ...
            case MISSION_MOVE_RIGHT:
                ...
                if (move_ticks >= MOVE_TICKS) {
                    printf("Mission: Right move complete.\n");
                    move_ticks = 0;
            ...
            case MISSION_MOVE_FORWARD:
                ...
                if (move_ticks >= MOVE_TICKS) {
                    printf("Mission: Forward move complete.\n");
                    move_ticks = 0;
            ...
            case MISSION_BRAKE_4:
                if (brake_ticks >= BRAKE_TICKS) {
                    brake_ticks = 0;
                    printf("Mission: Movement Done. Landing...\n");
                    //cpxPrintToConsole(LOG_TO_WIFI, "Mission: Movement Done. Landing...\n");
            ...
            case MISSION_DISARM:
                if (land_ticks >= LAND_TICKS) {
                    ...
                    mavlink_send_rc_override_4ch(STM32_SYSTEM_ID, STM32_COMPONENT_ID, 1500, 1500, 1000, 1500);     
        ...
        vTaskDelay(MISSION_DT_MS / portTICK_PERIOD_MS); // Loop 40Hz
    }
}
