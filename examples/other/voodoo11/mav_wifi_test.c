...
const char* mission_state_to_string(MissionState_t state) {
    switch (state) {
        ...
        case MISSION_HOVER_2:               return "HOVER_2";
...
void autonomous_mission_task(void *parameters) {
    ...
    const int MOVE_LEFT_TIME_MS = 500;
    const int MOVE_LEFT_TICKS = (MOVE_LEFT_TIME_MS / MISSION_DT_MS);
    const int LAND_TIME_MS = 2000;
    const int LAND_TICKS = (LAND_TIME_MS / MISSION_DT_MS);        
    static int hover_ticks = 0;
    static int move_left_ticks = 0;
    ...
    while (1) {
        ...
        switch (g_mission_state) {   
            ...
            case MISSION_MOVE_LEFT:
                ...
                if (move_left_ticks >= MOVE_LEFT_TICKS) {
                    printf("Mission: Left move complete. Braking...\n");
                    g_mission_state = MISSION_HOVER_2;
                }
                else {
                    move_left_ticks++;
                }
                break;
            case MISSION_HOVER_2:
                if (hover_ticks >= HOVER_TICKS) {
                    hover_ticks = 0;
                    printf("Mission: Hover complete. Landing...\n");
