 ...
 */
 void mavlink_init_data_streams(void) {
    ...
    // Request OPTICAL_FLOW_RAD at 20 Hz
    mavlink_request_data_stream(MAVLINK_MSG_ID_OPTICAL_FLOW, 20.0f); 
    
    // Request IMU at 20 Hz
    mavlink_request_data_stream(MAVLINK_MSG_ID_RAW_IMU, 20.0f); 
}
        ...
        if (mavlink_parse_char(MAVLINK_COMM_0, rx_byte, &received_msg, &status)) {
            ...
            case MAVLINK_MSG_ID_STATUSTEXT: {
                ...
                case MAVLINK_MSG_ID_OPTICAL_FLOW: {
                    ...
                    mavlink_msg_optical_flow_decode(&received_msg, &flow);
                    ...
                    int bytes_written = snprintf(&g_wifi_log_buffer[g_wifi_log_buffer_idx],
                        ...
                        "OF,%lu,%.4f,%.4f,%d,%.4f\n",
                        (unsigned long)flow.time_usec, flow.flow_rate_x, flow.flow_rate_y,
                        flow.quality, flow.ground_distance);
                ...
                case MAVLINK_MSG_ID_RAW_IMU: {
                    ...
                    mavlink_msg_raw_imu_decode(&received_msg, &imu);
                
                    // Write the IMU data to the buffer as well
                    int bytes_written = snprintf(&g_wifi_log_buffer[g_wifi_log_buffer_idx],
                        WIFI_LOG_BUFFER_SIZE - g_wifi_log_buffer_idx,
                        "IMU,%lu,%d,%d,%d,%d\n",
                        (unsigned long)imu.time_usec, imu.xgyro, imu.ygyro, imu.xacc, imu.yacc);
