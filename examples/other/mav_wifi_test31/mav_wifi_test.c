...
const char* mission_state_to_string(MissionState_t state) {
    switch (state) {
        ...
        case MISSION_GOTO_FORWARD:        return "GOTO_FORWARD";
        case MISSION_WAIT_FOR_ARRIVAL:    return "WAIT_FOR_ARRIVAL";
...
void autonomous_mission_task(void *parameters) {
    ...
    MissionState_t current_state = MISSION_WAIT_FOR_FC_CONNECT;
    ...
    int arrival_retries = 0;
    const int MAX_ARRIVAL_RETRIES = 20; // 20 retries * 500ms = 10 second timeout    

    cpxPrintToConsole(LOG_TO_WIFI, "Mission task started. Waiting for start command via WiFi...\n");

    while (current_state != MISSION_DONE) {
        cpxPrintToConsole(LOG_TO_WIFI, "Mission State: %s\n", mission_state_to_string(current_state));
        switch (current_state) {
            case MISSION_WAIT_FOR_FC_CONNECT:
                if (g_fc_connected) {
                    ...
                    current_state = MISSION_WAIT_FOR_STREAMS;
                } else {
                    vTaskDelay(500 / portTICK_PERIOD_MS);
            ...
            case MISSION_WAIT_FOR_STREAMS:
                if (g_pos_stream_active) {
                    cpxPrintToConsole(LOG_TO_WIFI, "Position data stream active.\n");
                    current_state = MISSION_WAIT_FOR_COMMAND;
                } else {
            ...
            case MISSION_WAIT_FOR_COMMAND:
                if (is_mission_start) {
                    ...
                    current_state = MISSION_COMMAND_ARM;
                }
                break;
            ...
            case MISSION_WAIT_FOR_ARM: {
                int result = mavlink_wait_for_ack(MAV_CMD_COMPONENT_ARM_DISARM, 1000); // Wait 1 second for ACK
                ...
                if (result == MAV_RESULT_ACCEPTED) {
                    ...
                    if (arm_retries >= MAX_ARM_RETRIES) {
                        ...
                        arm_retries++;
                        current_state = MISSION_COMMAND_ARM; // Go back to re-send the command
                    }
                }
                break;
            }
            ...
            case MISSION_GOTO_FORWARD:
                ...
                cpxPrintToConsole(LOG_TO_WIFI, "Mission: Target position command sent.\n");
                arrival_retries = 0; // Reset the arrival timeout counter

                current_state = MISSION_WAIT_FOR_ARRIVAL;

                break;
            case MISSION_WAIT_FOR_ARRIVAL: {
                // Check if the drone is close to the target position.
                // We'll check the 'East' position (y-axis) with a 10% tolerance.
                float current_pos_east = state->position_ned[1];
                float target_pos_east = FORWARD_DIST;
                float tolerance = target_pos_east * 0.10f; // 10% tolerance

                if (current_pos_east >= (target_pos_east - tolerance)) {
                    cpxPrintToConsole(LOG_TO_WIFI, "Mission: Arrived at target waypoint.\n");
                    current_state = MISSION_SET_MODE_RTL;
                } else {
                    // If not at the target, check for a timeout
                    if (arrival_retries >= MAX_ARRIVAL_RETRIES) {
                        cpxPrintToConsole(LOG_TO_WIFI, "Mission: FAILED to reach target after %d seconds. Aborting.\n", MAX_ARRIVAL_RETRIES / 2);
                        current_state = MISSION_DONE;
                    } else {
                        cpxPrintToConsole(LOG_TO_WIFI, "Mission: Flying to target... Current East Pos: %.2f\n", current_pos_east);
                        arrival_retries++;
                    }
                }
                break;
        ...
        vTaskDelay(1000 / portTICK_PERIOD_MS); // Loop twice per second
    }
    vTaskDelete(NULL);
}
