        ...
        switch (g_mission_state) {
            ...
            case MISSION_PERFORM_TAKEOFF_CLIMB: {
                ...
                if (state->altitude_m >= TAKEOFF_ALT) {
                    ...
                    // We are still climbing. Command a gentle ascent using MANUAL_CONTROL.
                    // A throttle value of 650 is ~65%, which is a good climb rate.
                    cpxPrintToConsole(LOG_TO_WIFI, "Climbing... (Alt: %.2f m)\n", state->altitude_m);
                    mavlink_send_manual_control(0, 0, 650, 0); // x=0, y=0, z=650 (throttle), r=0
                }
                break;
            }
            case MISSION_HOVER:
